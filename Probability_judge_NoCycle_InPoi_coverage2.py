...
if __name__ == '__main__':
    ...
    needpartK = needK/5
    countlist = []
    kp = 0.2 # 原本是 0.0001 不過這裡放大 10000 倍
    low = 0
    lowlist = []
    largelist = []
    coverageAll = []
    ...
    for i in range(100) : #做100次
        ...
        for ii in range(5) : #5個time slot
            ...
            if ii == 4:
                ...
                deltaP2 = ((1/coverage) * (Newcoverage * lifenum + (needK - lifenum)) + deltaP) * kp
                p = poi.Pcal(deltaP2) #round 四捨五入
                print("newcoverage:", Newcoverage)
                print("deltaP2:", deltaP2)
                print("deltaP3:", Newcoverage * lifenum + (needK - lifenum))
                print(p)
            elif ii < 4:
                deltaP = deltaP + nextP
